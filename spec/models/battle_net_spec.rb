...
describe BattleNet, type: :model do
  ...
  describe 'with guild parameters' do
    before { @battle_net = BattleNet.new(region: 'US', guild: 'Vox Immortalis', realm: 'Hyjal', auto_connect: true) }
    subject { @battle_net }
    ...
    describe 'with invalid realm' do
      before { @battle_net = BattleNet.new(region: 'US', guild: 'Vox Immortalis', realm: 'Hyjaaaaal', auto_connect: true) }

      specify { expect(@battle_net.errors[:battle_net_error].first).to eq 'Realm not found.' }
    end

    describe 'with invalid guild' do
      before { @battle_net = BattleNet.new(region: 'US', guild: 'Vooooooox Immmmmmmmortalis', realm: 'Hyjal', auto_connect: true) }

      specify { expect(@battle_net.errors[:battle_net_error].first).to eq 'Guild not found.' }
    end

    describe 'with invalid region' do
      before { @battle_net = BattleNet.new(region: 'AZ', guild: 'Vox Immortalis', realm: 'Hyjal') }
      subject { @battle_net }
  ...
  describe 'with character parameters' do
    before { @battle_net = BattleNet.new(region: 'US', character_name: 'Kulldar', realm: 'Hyjal', type: 'character', auto_connect: true) }
    subject { @battle_net }
    ...
    describe 'with invalid character name' do
      before { @battle_net = BattleNet.new(region: 'US', character_name: 'Kulllllldar', realm: 'Hyjal', type: 'character', auto_connect: true) }
